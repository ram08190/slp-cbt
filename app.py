...
# 🌟 [경로 세척 함수] D드라이브 경로를 깃허브 상대 경로로 강제 변환
# Base64 변환 없이 파일 경로만 텍스트로 전달하여 에러를 방지합니다.
def get_clean_path(img_path):
...
if mode == "📝 시험 시작":
    ...
    for _, row in df.iterrows():
        try:
            ...
            q_obj = {
                ...
                "img": get_clean_path(clean_val(row.get('img', ''))), # 👈 경로만 전달
                "options": [{"text": clean_val(row.get(f'option{i}', '')), "img": get_clean_path(clean_val(row.get(f'opt_img{i}', '')))} for i in range(1, 6)]
            }
    ...
    if os.path.exists(HTML_FILE):
        ...
        # 🌟 HTML 데이터를 최소화하여 TypeError 방지
        st.components.v1.html(final_html, height=1200, scrolling=True)

# ---------------------------------------------------------
# 모드 2: 문항 관리 (기존 기능 100% 복구)
# ---------------------------------------------------------
elif mode == "🛠️ 문항 관리":
    ...
    tab1, tab2 = st.tabs(["📄 문제 지문/사례/이미지", "🔢 보기/정답"])
    with tab1:
